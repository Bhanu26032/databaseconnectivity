...
app.post('/register1', encodeUrl, (req, res) => {
    ...
    con.connect(function(err) {
        ...
        // checking if the user already registered or not in the database 
        con.query("SELECT * FROM register.register WHERE email = ? AND password  = ?",[email,password], function(err, result){
            if(err){
                console.log(err);
            };
            ...
            function userPage(){
                // We create a session for the Home page (user page) and save the user data to this session:
                req.session.user = {
                    firstname: firstname,
                    lastname: lastname,
                    gender: gender,
                ...
                var sql = "INSERT INTO register.register (firstname, lastname, gender, email, password, address, nationality, state, city, pincode) VALUES (?,?,?,?,?,?,?,?,?,?)";
    ...
    con.connect(function(err) {
        ...
        con.query("SELECT * FROM register.register WHERE email = ? AND password = ?",[email,password], function (err, result) {
          if(err){
           ...
           function userPage(){
            // We create a session for the Homepage (user page) and save the user data to this session:
            req.session.user = {
